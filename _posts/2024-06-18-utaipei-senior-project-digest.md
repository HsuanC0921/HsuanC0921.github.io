...
<!--
大型語言模型, LLM, 微調, 合併, 資訊專題, 資訊科學系, 專題報告, 北市大, 臺北市立大學, utaipei, 自然語言處理, NLP, 語言模型, 程式設計助手, GitHub Copilot, LLAMA, llama.cpp, Langchain, 人工智慧, AI, Transformer, GPT, ChatGPT, Mistral, Breeze, Dolphin, SLERP, 量化, Oracle Cloud, Streamlit, Chainlit, Open WebUI, Ollama, HumanEval, MBPP
-->
...
由於當初未參加資訊專題競賽決賽，為了確保專題報告內容的公開性，我將報告內容的精簡版本製作成透過Github Pages所架設的Blog的Markdown文章，供網際網路上的任何人公開閱覽。


本篇Markdown文章介紹了我在臺北市立大學資訊科學系的資訊專題報告研究，標題為「LLM應用實戰」。

這項專題研究最初計劃對Mistral 7B大型語言模型進行中文微調，但由於大規模微調需要強大的硬體資源且花費相當可觀，我改為嘗試透過合併的方式建立一個能理解中文的程式助手。

為了找到符合我要求且效果良好的模型，我從數百個不同的公開LLM模型中一一挑選並進行實際測試。

最終選出了聯發科的Breeze 7B和Cognitive Computation的dolphin-2.6-mistral-7b-dpo-laser這兩個模型，並透過SLERP(球面線性插值)技術進行合併，最終獲得了我稱之為BreezeDolphin的模型。

為了驗證模型效果和後續投入使用，我前後使用Streamlit和Chainlit撰寫充當AI聊天介面的前端UI，在多次測試中也發現BreezeDolphin具備一定的程式設計能力，但在中文理解和生成完整結果方面還有改進空間。最終，我將模型部署在Oracle Cloud上以方便隨時隨地使用。

在研究結論中，我指出，儘管BreezeDolphin有潛力成為不錯的程式助手，但需要收集更多高品質語料、改進生成能力、探索新模型架構訓練方法等，才能進一步提升後續型號模型的表現。
